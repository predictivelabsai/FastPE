"""Export endpoints — XLS for tables, DOCX for memos, Plotly charts.
...
@rt("/app/export/xlsx", methods=["POST"])
async def export_xlsx(request: Request):
    ...
    rows = data.get("rows", [])
    title = data.get("title", "PEHero Export")
    ...
    header_fill = PatternFill(start_color="1F5D43", end_color="1F5D43", fill_type="solid")
    header_align = Alignment(horizontal="center", vertical="center")
    thin_border = Border(
        left=Side(style="thin", color="E3DFD2"),
        right=Side(style="thin", color="E3DFD2"),
        top=Side(style="thin", color="E3DFD2"),
        bottom=Side(style="thin", color="E3DFD2"),
    )
    ...
    filename = f"pehero-{safe_title}.xlsx"
...
@rt("/app/export/docx", methods=["POST"])
async def export_docx(request: Request):
    form = await request.form()
    markdown = form.get("markdown", "")
    title = form.get("title", "PEHero Document")
    ...
    style.font.color.rgb = RGBColor(0x14, 0x23, 0x1B)

    title_para = doc.add_heading(title, level=1)
    title_para.alignment = WD_ALIGN_PARAGRAPH.LEFT
    for run in title_para.runs:
        run.font.color.rgb = RGBColor(0x1F, 0x5D, 0x43)
    ...
    filename = f"pehero-{safe_title}.docx"
...
async def generate_chart(request: Request):
    ...
    fig.update_layout(
        ...
        font=dict(family="Inter, Calibri, sans-serif", size=12, color="#14231B"),
        title=dict(font=dict(size=16, color="#1F5D43")),
        plot_bgcolor="#F7F6F1",
    ...
    if cat_col and len(numeric_cols) == 1:
        ...
        if len(rows) <= 8:
            fig = go.Figure(data=[go.Pie(
                ...
                marker=dict(colors=_pe_palette(len(rows))),
            )])
            fig.update_layout(title=title)
            return "pie", fig
        else:
...
def _pe_palette(n):
    base = [
        "#1F5D43", "#2F7151", "#4A8E66", "#7A9E88",
        "#C89B5B", "#B57D3E", "#6B4E2F", "#9C8F7A",
        "#3C5748", "#9CA89E",
    ]
