/* PEHero — Instructions prompt editor (Quill WYSIWYG + Markdown + History) */

var instrQuill = null;
var instrActiveTab = 'editor';

// ── Initialise Quill on page load ────────────────────────────────────
...
function switchTab(tab) {
    ...
    if (tab === 'editor') {
        ...
        // Sync current state before showing history
        if (instrActiveTab === 'editor' && instrQuill) {
...
function quillToMarkdown() {
    ...
    for (var i = 0; i < ops.length; i++) {
        ...
        // Handle line-level formatting (headers, lists)
        if (text === '\n') {
            if (attrs.header) {
...
async function savePrompt() {
    ...
    // Sync active tab content to src
    if (instrActiveTab === 'editor' && instrQuill) {
        src.value = quillToMarkdown();
    } else if (instrActiveTab === 'markdown' && mdTextarea) {
        src.value = mdTextarea.value;
    ...
    try {
        ...
        var resp = await fetch('/skills/' + encodeURIComponent(slug), {
            method: 'POST',
